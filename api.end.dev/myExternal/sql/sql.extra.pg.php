<?php
echo PHP_EOL . PHP_EOL . "delete from \"default\".system_data;" . PHP_EOL;

echo "/*平台*/" . PHP_EOL;

$platform = array(
    array('key' => 'admin', 'label' => '管理员后台端'),
    array('key' => 'store', 'label' => '商户端'),
    array('key' => 'app', 'label' => 'APP端'),
    array('key' => 'wx', 'label' => '微信端'),
    array('key' => 'pos', 'label' => 'POS端'),
);
echo "insert into \"default\".system_data(key,name,data) values ('platform','平台','" . json_encode($platform) . "');" . PHP_EOL;
...
echo "insert into \"default\".system_data(key,name,data) values ('permission','权限','" . json_encode($permission) . "');" . PHP_EOL;
...
$system_config = array(
    ...
    array(
        'key' => 'order_sent_expire_day',
        'name' => '卖家发货期限（天）',
        'value' => 7,
    ),
    array(
        'key' => 'order_received_expire_day',
        'name' => '买家收货期限（天）',
        'value' => 15,
    ),
    array(
        'key' => 'order_evaluation_expire_day',
        'name' => '买家评价期限（天）',
        'value' => 30,
    ),
    array(
        'key' => 'order_refund_received_expire_day',
        'name' => '买家售后收货期限（天）',
        'value' => 15,
    ),
);
echo "insert into \"default\".system_data(key,name,data) values ('system_config','系统配置','" . json_encode($system_config) . "');" . PHP_EOL;
...
echo "insert into \"default\".system_data(key,name,data) values ('path','限制性路径','" . json_encode($path) . "');" . PHP_EOL;
...
echo "insert into \"default\".system_data(key,name,data) values ('asset_file_size_unit','文件大小单位','" . json_encode($asset_file_size_unit) . "');" . PHP_EOL;
...
echo "insert into \"default\".system_data(key,name,data) values ('asset_content_type','资源许可文件格式','" . json_encode($asset_content_type) . "');" . PHP_EOL;

echo "/*反馈问题类型*/" . PHP_EOL;
$feedback_type = array(
    array('key' => 'pay', 'label' => '支付问题'),
    array('key' => 'order', 'label' => '订单问题'),
    array('key' => 'goods', 'label' => '商品问题'),
    array('key' => 'account', 'label' => '账号问题'),
    array('key' => 'recharge', 'label' => '充值问题'),
    array('key' => 'other', 'label' => '其他'),
);
echo "insert into \"default\".system_data(key,name,data) values ('feedback_type','反馈问题类型','" . json_encode($feedback_type) . "');" . PHP_EOL;
