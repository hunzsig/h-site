...
CREATE TYPE "default".is_sure AS ENUM('-1', '1');
CREATE TYPE "default".is_enable AS ENUM('-10','-1','1');
/*时间单位*/
CREATE TYPE "default".datetime_unit AS ENUM('year','month','week','day','hour','minute');
/*用户实名状态 -1未认证 -2未通过 1认证中 10已认证*/
CREATE TYPE "default".identity_auth_status AS ENUM('-1','-2','1','10');
/*用户状态 -10被注销 -5被冻结 -2未通过 -1未审核 1正常 */
CREATE TYPE "default".user_status AS ENUM('-10','-5','-2','-1','1');
/*用户性别 -1未设置 1男 2女 */
CREATE TYPE "default".user_sex AS ENUM('-1','1','2');
...
CREATE TABLE "default".user (
  ...
  status "default".user_status NOT NULL DEFAULT '-1',
  inviter_uid bigint,
  login_pwd text,
  ...
  identity_auth_status "default".identity_auth_status NOT NULL DEFAULT '-1',
...
CREATE TABLE "default".user_info (
  uid bigint NOT NULL,
  sex "default".user_sex NOT NULL DEFAULT '-1',
  birthday date,
  nickname text,
  avatar jsonb,
  PRIMARY KEY (uid)
